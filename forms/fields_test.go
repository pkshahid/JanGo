...
func TestFields(t *testing.T) {
	...
	if err == nil {
		t.Errorf("Expected MaxLength error")
	}

	cleanCf, _ := cf.Clean("  test  ")
	if cleanCf != "test" {
		t.Errorf("Expected striped 'test', got '%s'", cleanCf)
	}
	...
	if err == nil {
		t.Errorf("Expected valid integer error")
	}

	cleanInt, _ := intf.Clean(" 42 ")
	if cleanInt != 42 {
		t.Errorf("Expected 42, got %v", cleanInt)
	}
	...
	if cleanFf != 3.14 {
		t.Errorf("Expected 3.14, got %v", cleanFf)
	}
	...
	if cleanBf != true {
		t.Errorf("Expected true, got %v", cleanBf)
	}

	_, err = bf.Clean("off")
	if err == nil {
		t.Errorf("Expected required error on false/off")
	...
	if cleanSf != "valid-slug_123" {
		t.Errorf("Expected valid-slug_123, got %s", cleanSf)
	}

	// URLField
	uf := &URLField{CharField{BaseField: BaseField{IsRequired: true}}}
	cleanUf, _ := uf.Clean("example.com")
	if cleanUf != "http://example.com" {
		t.Errorf("Expected http://example.com, got %s", cleanUf)
	}
	...
	if cleanChf != "A" {
		t.Errorf("Expected A, got %s", cleanChf)
	}
	...
	if strings.Join(cleanMcf.([]string), ",") != "A,B" {
		t.Errorf("Expected A,B")
	}

	_, err = mcf.Clean([]string{"A", "C"})
	if err == nil {
		t.Errorf("Expected invalid choice error for C")
	}
}
