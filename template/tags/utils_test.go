...
func TestUtilTags(t *testing.T) {
	...
	if resWith != "12" {
		t.Errorf("With tag mismatch: %s", resWith)
	}
	...
	if resComment != "AB" {
		t.Errorf("Comment tag mismatch: %s", resComment)
	}
	...
	if err2 != nil {
		t.Errorf("Render error: %v", err2)
	}
	if resCycle2 != "XYX" {
		t.Errorf("Cycle tag mismatch: %q", resCycle2)
	}
	...
	if len(resNow) != 4 || !strings.HasPrefix(resNow, "20") {
		t.Errorf("Now tag mismatch: %s", resNow)
	}
	...
	if resSpaceless != expectedSpaceless {
		t.Errorf("Spaceless mismatch:\n%q\nExpected:\n%q", resSpaceless, expectedSpaceless)
	}
	...
	if resVerbatim != "{{ dont_render }}" {
		t.Errorf("Verbatim mismatch: %s", resVerbatim)
	}
	...
	if !strings.HasPrefix(resLorem, "Lorem ipsum") {
		t.Errorf("Lorem mismatch: %s", resLorem)
	}
}
