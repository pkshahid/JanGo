...
func (p *TimerPanel) ProcessResponse(req *godjangohttp.Request, resp godjangohttp.Response) {
	p.End = time.Now()
}
...
func (p *RequestPanel) RenderContent() string {
	...
	for k, v := range p.Req.GET {
		sb.WriteString(fmt.Sprintf("<li>%s: %v</li>", k, v))
	}
	sb.WriteString("</ul><h3>POST Parameters</h3><ul>")
	if p.Req.POST != nil {
		for k, v := range p.Req.POST {
			sb.WriteString(fmt.Sprintf("<li>%s: %v</li>", k, v))
		}
	}
	sb.WriteString("</ul><h3>Cookies</h3><ul>")
	for k, v := range p.Req.COOKIES {
		sb.WriteString(fmt.Sprintf("<li>%s: %v</li>", k, v))
	}
	sb.WriteString("</ul>")
	return sb.String()
...
func (p *RoutingPanel) RenderContent() string {
	if p.Req == nil || p.Req.ResolverMatch == nil {
		return "<p>No routing match found.</p>"
	}
	var sb strings.Builder
